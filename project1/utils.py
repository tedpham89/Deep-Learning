...
def load_mnist_trainval():
    ...
    # print("train data single", len(train_data[0]))
    # print("train_data count: ", len(train_data))
    # print("train_label count: ", len(train_label))
    # print("val_data count: ", len(val_data))
    # print("val_label count: ", len(val_label))
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    # shuffle the data
    shuffle_indices = list(range(len(data)))  #original indices
    if shuffle:
    ...
    label = [label[i] for i in shuffle_indices]
    
    # Separate data into batches
    for i in range(0, len(data), batch_size):
        # temp_data = data[i:i:batch_size]
        temp_data = np.array(data[i: i + batch_size])
        temp_label = np.array(label[i: i + batch_size])

        batched_data.append(temp_data)
        batched_label.append(temp_label)

    # print("batch_data ",len(batched_data))
    # print("batch_data size ",len(batched_data[0]))
    # print("batch_label ",len(batched_label))
