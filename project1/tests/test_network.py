...
class TestNetwork(unittest.TestCase):
    ...
    def test_one_layer_softmax_relu(self):
        ...
        self.assertAlmostEqual(expected_loss, loss, places=5,msg="False1")
        # print("expgrad",expected_grad.shape,"sum", np.sum(expected_grad))
        # print("w_grad",w_grad.shape,"sum", np.sum(w_grad))
        diff = np.sum(np.abs(expected_grad - w_grad))
        self.assertAlmostEqual(diff, 0,msg="False2")
    ...
    def test_two_layer_net(self):
        ...
        self.assertAlmostEqual(expected_loss, loss, places=5,msg="False3")
        
        # print(np.sum(w1_grad_expected))
        # print(np.sum(model.gradients['W1']))
